...
#'@return A `ts_regswmulti` object (S3) to be extended by concrete models.
#'@examples
#'# Abstract base class for sliding-window regressors
#' # Use concrete subclasses such as ts_mlp(), ts_rf(), ts_svm(), ts_elm()
#'@export
ts_regswmulti <- function(preprocess=NA, input_size=NA) {
  obj <- ts_reg()
  ...
  class(obj) <- append("ts_regswmulti", class(obj))
  return(obj)
}

#'@exportS3Method fit ts_regswmulti
#'@inheritParams do_fit
#'@return A fitted object with learned backend model and fitted preprocessor.
fit.ts_regswmulti <- function(obj, x, y, ...) {
...
#'@exportS3Method predict ts_regswmulti
...
predict.ts_regswmulti <- function(object, x, steps_ahead=1, ...) {
  if (steps_ahead == 1) {
...
#'@exportS3Method do_predict ts_regswmulti
#'@inheritParams do_predict
#'@return Numeric vector with predictions using the backend's `predict`.
do_predict.ts_regswmulti <- function(obj, x) {
